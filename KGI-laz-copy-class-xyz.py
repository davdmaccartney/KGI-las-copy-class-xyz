...
for filename in os.listdir(dirname1):
     if filename.endswith(".las"):
        ...
        # make a copy of the target points
        point_copy_ori = inFile1.points.copy()

        # compute any ofsset difference to the target file
        hXdif=0
        hYdif=0
        hZdif=0
        ...
        # select the layers needed from the target to be modified fron the source file
        ...
        # creating the Panda array
        
        ori = pd.DataFrame(np.empty(0, dtype=[('X',np.int32),('Y',np.int32),('Z',np.int32),('intensity',np.int),('flag_byte',np.int),('raw_classification',np.int),('angle',np.ubyte)]))
        src = pd.DataFrame(np.empty(0, dtype=[('X',np.int32),('Y',np.int32),('Z',np.int32),('intensity',np.int),('flag_byte',np.int),('raw_classificationT',np.int),('angle',np.ubyte)]))
        ...
        #if anny offset difference to be aplied
        ...
        # drop duplicated rows in order to keep the same number of line in the target file
        src.drop_duplicates(subset =['X','Y','Z','intensity','flag_byte','angle'], keep=False,inplace=True)


        # merge left join style
        #start = timer()
